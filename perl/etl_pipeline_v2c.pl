...
#    $outfile=&add_slash($temppath).$settings{'metadata'}{'structuresAffected'}."_".$temp[0]."_start_upload.csv";
    my $key=$settings{'metadata'}{'structuresAffected'}."_".$settings{'metadata'}{'inFileName'};
    
        $key=~ s/\.csv//;
    $outfile=&create_filename($temppath,$key."_upload","csv","start");
...
sub create_etl
{
   ...
   my $key=$settings{'metadata'}{'structuresAffected'}."_".$settings{'metadata'}{'inFileName'};
        ...
        $key=~ s/\.csv//;
   my $outfile=&create_filename($tempf,$key,"json",$flag);
    ...
    {
        $command.="\'/Insert/d; /increment/d; /status/d; /skipDuplicates/d;".$subcomm."s/fpathValue/".$s."/g;"."' <$etltemp"." >$outfile";
    }
